...
// HELPER FUNCTION PARA SA BOUQUET
...
if ($data && isset($_SESSION['user_id'])) {
    ...
    // materials_deducted = 1 - ibinabawas na rito ang materials sa ibaba, kaya
    // hindi na dapat ulitin ng admin_orders.php pag-lipat sa "To Ship".
    $query = "INSERT INTO orders (user_id, order_type, size, total_price, custom_image, items_json, status, materials_deducted) VALUES (?, ?, ?, 0, ?, ?, ?, 1)";
    ...
    if ($stmt->execute()) {
        ...
        if ($order_type === 'Custom Bouquet') {
            ...
            if (is_array($items_array)) {
                foreach ($items_array as $item) {
                    ...
                    // hindi ito tunay na bulaklak kaya hindi dapat ibawas sa materials.
        ...
        elseif ($order_type === 'Custom T-Shirt') {
            ...
            // EXACT WORD MATCH: Para hindi madamay ang "Colored" kapag pinili ang "Red"
            $c_kw = $conn->real_escape_string(strtolower(trim($shirt_color_name)));
            $ink_ml = rand(8, 15);

            $sql_s = "UPDATE materials SET stock = stock - 1 
                      WHERE (LOWER(category) = 'shirt' OR LOWER(name) LIKE '%shirt%') 
                      AND (LOWER(name) = '$c_kw' 
                           OR LOWER(name) LIKE '$c_kw %' 
                           OR LOWER(name) LIKE '% $c_kw' 
                           OR LOWER(name) LIKE '% $c_kw %') 
                      LIMIT 1";
            $conn->query($sql_s);

            $sql_i = "UPDATE materials SET stock = stock - $ink_ml WHERE LOWER(name) LIKE '%cmyk ink%' LIMIT 1";
            $conn->query($sql_i);
        }
