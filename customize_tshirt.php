...
// Hindi kailangang ibalik ang apat na snapshot - ginagawa ulit ang mga iyon ng
// requestShirt() sa bawat save. Ang mahalaga ay ang items at ang kulay.
$EDIT_DATA = isset($_GET['edit']) ? build_edit_data($_GET['edit'], 'Custom T-Shirt') : null;
?>
...
const SHIRT_PRICES = { 'Small': 259, 'Medium': 279, 'Large': 299, 'Extra Large': 319 };
...
async function requestShirt() {
    ...
    try {
        ...
        payloadItems.unshift({
            id: 'shirt_color_info',
            type: 'info',
            // HUWAG baguhin ang format ng 'name' - hinihiwa ito ng my_orders.php
            // gamit ang str_replace('Shirt Color: ', '') para maibalik ang stock.
            name: 'Shirt Color: ' + state.shirtColorName,
            hex: state.shirtColor,
            qty: 1
        });
        ...
        const orderData = {
            ...
            edit_id: EDIT_DATA ? EDIT_DATA.cartId : null
        };
        ...
        if (result.success) {
            alert(result.replaced ? "Cart item updated!" : "Custom Shirt saved to cart!");
 ...
 * Mas simple kaysa sa bouquet: hindi nililinis ng setSize() dito ang state.items.
 * May sariling 'view' ang bawat item kaya babalik sila sa tamang bahagi ng shirt.
 */
function applyEditData() {
    if (!EDIT_DATA) return;

    const key = SHIRT_PRICES[EDIT_DATA.sizeKey] ? EDIT_DATA.sizeKey : 'Medium';
    setSize(key, SHIRT_PRICES[key]);

    // Kapag lumang cart item na walang naka-save na hex, hinuhulaan ito mula sa pangalan.
    setShirtColor(EDIT_DATA.shirtColor || '#FFFFFF', EDIT_DATA.shirtName || 'White');
