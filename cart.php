...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['checkout_selected'])) {
    ...
    foreach ($custom_ids as $cid) {
        if (isset($_SESSION['cart'][$cid])) {
            ...
            // materials_deducted = 1 dahil ibinabawas na natin ang materials sa ibaba.
            // Ito ang pumipigil sa admin_orders.php na bawasan ulit pag-lipat sa "To Ship".
            $query = "INSERT INTO orders (user_id, order_type, size, total_price, custom_image, items_json, status, materials_deducted) VALUES (?, ?, ?, 0, ?, ?, 'Pending', 1)";
                ...
                if ($order_type === 'Custom Bouquet') {
                    ...
                    if (is_array($items_array)) {
                        foreach ($items_array as $item) {
                            // Ang wrapper_info / custom_size_info / deduction_data ay metadata lang -
                            // hindi ito tunay na bulaklak kaya hindi dapat ma-insert sa order_items
                            // o hanapin sa flower_materials.
...
<head>
    ...
    <style>
        ...
        .edit-design-btn { display: inline-block; color: var(--coral); border: 1px solid var(--peach); text-decoration: none; font-size: 9px; font-weight: 900; text-transform: uppercase; letter-spacing: 0.5px; padding: 6px 11px; border-radius: 50px; margin-right: 8px; transition: 0.3s; }
        .edit-design-btn:hover { background: var(--coral); color: #fff; border-color: var(--coral); }
    </style>
                            ...
                            if (strpos($id, 'custom_') === 0) {
                                ...
                                <tr>
                                    ...
                                    <td style="white-space: nowrap;">
                                        <?php
                                            // Habang nasa cart pa, pwede pang baguhin ang design.
                                            $editor = ($data['order_type'] === 'Custom T-Shirt') ? 'customize_tshirt.php' : 'customizer.php';
                                        ?>
                                        <a href="<?= $editor ?>?edit=<?= urlencode($id) ?>" class="edit-design-btn" title="Edit this design">Edit Design</a>
                                        <a href="?remove=<?= $id ?>" class="remove-icon" title="Remove Item">&times;</a>
                                    </td>
                                </tr>
                                <?php
                            } 
