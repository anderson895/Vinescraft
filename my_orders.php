...
<head>
    ...
    <style>
        ...
        /* Payment / delivery summary strip sa bawat order card */
        .pay-strip { display: flex; flex-wrap: wrap; gap: 6px; margin: 10px 0 8px; }
        .pay-strip span { font-size: 9px; font-weight: 900; text-transform: uppercase; letter-spacing: 0.5px; color: #8a8a8a; background: #f4f4f4; padding: 4px 9px; border-radius: 50px; }
    ...
    <?php if($orders->num_rows > 0): while($row = $orders->fetch_assoc()): ?>
        ...
        <div class="order-card">
            ...
            <div class="order-info">
                    ...
                    if (!empty($row['items_json'])) {
                        ...
                        if (is_array($items)) {
                            ...
                            foreach ($items as $it) {
                                ...
                                // Internal lang ang wrapper_info. Sinasadyang hindi kasama dito
                                // ang custom_size_info para makita pa rin ang "Custom Request: N inches".
                                if(isset($it['id']) && $it['id'] === 'wrapper_info') continue;
                                $n = $it['name'] ?? 'Item';
                ...
                <?php
                    // Ang grand_total ay naka-lock sa checkout. Ang mga lumang order bago
                    // pa ito idagdag ay NULL, kaya kino-compute na lang natin ulit.
                    $grand   = $row['grand_total'] !== null
                             ? floatval($row['grand_total'])
                             : floatval($row['total_price']) + floatval($row['shipping_fee']);
                ...
                <div class="pay-strip">
                    ...
                    <?php if($row['payment_plan'] === 'dp50'): ?><span>Downpayment</span><?php endif; ?>
                    <?php if(!empty($row['courier'])): ?><span><?= htmlspecialchars($row['courier']) ?></span><?php endif; ?>
                    <?php if(floatval($row['shipping_fee']) > 0): ?><span>+₱<?= number_format($row['shipping_fee'], 2) ?> shipping</span><?php endif; ?>
                </div>
                <?php endif; ?>

                <div style="font-size: 18px; font-weight: 900;">
                    <?= $row['status'] == 'Pending' ? '<span style="color:#ccc; font-size:12px;">Pricing...</span>' : '₱' . number_format($grand, 2) ?>
                </div>
                ...
                <?php elseif($row['payment_status'] === 'COD' && !in_array($row['status'], ['Cancelled', 'Completed'])): ?>
                    <div style="font-size:11px; color:#a2695c; margin-top:4px;">
                        <strong>₱<?= number_format($balance, 2) ?> due on delivery</strong> (Cash on Delivery)
                    </div>
                <?php endif; ?>
                ...
                <?php elseif($row['status'] == 'To Pay'): ?>
                    <div style="display: flex; gap: 10px; margin-top: 10px;">
                        <button class="btn-action" style="margin-top: 0; flex: 1;" onclick="window.location.href='checkout.php?id=<?= $row['order_id'] ?>'">Checkout &amp; Pay</button>
