...
// RE-EDIT: kapag may ?edit=<cart key>, ibabalik ang naka-save na design mula sa cart.
// Metadata lang ang ipinapasa sa JS - hindi kasama ang base64 na larawan.
$EDIT_DATA = isset($_GET['edit']) ? build_edit_data($_GET['edit'], 'Custom Bouquet') : null;
...
const EDIT_DATA = <?php echo json_encode($EDIT_DATA); ?>;
...
function addItem(src, type, name, isDynamic = false) {
    ...
    // Nagbabanggaan ang ID kapag dalawang item ang naidagdag sa iisang millisecond.
    let newItem = { id: Date.now() * 1000 + Math.floor(Math.random() * 1000), type: type, name: name, file: src, x: 125, y: 150, w: sizeVal, h: sizeVal, rot: 0, bend: 0, flip: 1, isDynamic: isDynamic };
...
function setWrapper(backUrl, frontUrl) {
    ...
    // Itinatago pareho para maisama sa items_json at maibalik kapag ni-edit sa cart.
    state.wrapperBack = backUrl;
    state.wrapperFront = frontUrl;
...
async function submitRequest() {
    ...
    try {
        ...
        // Kailangan itong i-save para maibalik ang design kapag ni-edit ulit sa cart.
        // Hindi kasi naka-store ang wrapper sa items dati, kaya hindi kayang buuin ulit.
        payloadItems.unshift({
            id: 'wrapper_info',
            type: 'info',
            name: 'Wrapper',
            back: state.wrapperBack || state.wrapper || null,
            front: state.wrapperFront || null
        });
        ...
        const orderData = {
            ...
            edit_id: (typeof EDIT_DATA !== 'undefined' && EDIT_DATA) ? EDIT_DATA.cartId : null
        };
        ...
        if(result.success) {
            alert(result.replaced ? "Cart item updated!" : "Custom Bouquet saved to cart!");
...
/**
 * Ibalik ang design na galing sa cart.
 *
 * Mahalaga ang pagkakasunod-sunod: nililinis ng setSize() ang state.items at ang
 * counts, at ito rin ang tumatawag sa renderWrappers(). Kaya size muna, tapos
 * wrapper, saka lang ang mga item.
 */
function applyEditData() {
    if (!EDIT_DATA) return;

    const key = RULES[EDIT_DATA.sizeKey] ? EDIT_DATA.sizeKey : 'Medium';

    // 1. Size - nililinis nito ang items at counts, at nirerender ang wrappers
    setSize(key, RULES[key].price);

    if (EDIT_DATA.customInches) {
        const input = document.getElementById('customSizeInput');
        if (input) input.value = EDIT_DATA.customInches;
    }

    // 2. Wrapper. Ang mga lumang cart item ay walang naka-save na wrapper,
    //    kaya default muna sa Style 1 para hindi blangko ang canvas.
    if (EDIT_DATA.wrapper && EDIT_DATA.wrapper.back) {
        setWrapper(EDIT_DATA.wrapper.back, EDIT_DATA.wrapper.front || EDIT_DATA.wrapper.back);
    } else {
        const prefix = RULES[key].prefix;
        setWrapper(`img/custom/bouquet/wrapper/${prefix}1.png`, `img/custom/bouquet/wrapper/t_${prefix}1.png`);
        const banner = document.getElementById('wrapperNotice');
        if (banner) banner.style.display = 'block';
    }

    // 3. Mga item, tapos bilangin ulit
    state.items = Array.isArray(EDIT_DATA.items) ? EDIT_DATA.items : [];
    counts = { flower: 0, filler: 0, leaf: 0 };
    ...
    const btn = document.getElementById('request-btn');
    if (btn) btn.innerText = "Update Cart Item";
}
