...
<head>
    ...
    <style>
        ...
        /* UPLOAD SECTION */
        .upload-section { padding: 20px 30px; background: #fff; text-align: center; border-top: 1px dashed #ccc; }
        .upload-section label { display: block; font-size: 11px; font-weight: 800; color: #0157ff; text-transform: uppercase; margin-bottom: 10px; }
        .upload-section input { width: 100%; font-size: 12px; padding: 10px; border: 1px solid #eee; border-radius: 8px; background: #fafafa; outline: none; }

        .btn-pay { display: block; width: 85%; margin: 10px auto 20px; background-color: #0157ff; color: white; border: none; padding: 16px; border-radius: 50px; font-size: 15px; font-weight: 900; cursor: pointer; transition: 0.3s; text-transform: uppercase; box-shadow: 0 5px 15px rgba(1, 87, 255, 0.3); }
        .btn-pay:hover:not(:disabled) { background-color: #0046cc; transform: translateY(-2px); }
        
        /* DISABLED STATE */
        .btn-pay:disabled { background-color: #ccc; cursor: not-allowed; box-shadow: none; color: #666; }
...
<div class="gcash-container">
    ...
    <form action="process_final_payment.php" method="POST" enctype="multipart/form-data">
        ...
        <div class="upload-section">
            ...
            <input type="file" name="receipt_image" id="receiptInput" accept="image/*" required>
        </div>

        <button type="submit" class="btn-pay" id="payBtn" disabled>I have Sent the Payment</button>
    </form>
...
<script>
    // JS Logic para i-enable ang button kapag may ini-upload na picture
